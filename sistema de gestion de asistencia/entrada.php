...
if (isset($data->employee_id) && isset($data->log_type) && isset($data->datetime_log)) {
    ...
    // Verificar si el status del empleado es 1
    ...
    if ($statusCheckResult && $statusCheckResult->num_rows > 0) {
        ...
        if ($row['status'] == 1) {
            echo json_encode(array("success" => false, "message" => "El empleado ya registró asistencia."));
            $conn->close();
            exit();
        }
    } else {
    ...
    if ($conn->query($insertQuery) === TRUE) {
        // Actualizar el status del empleado a 1
        $updateStatusQuery = "UPDATE employee SET status = 1 WHERE id = '$employee_id'";
        $conn->query($updateStatusQuery);

        echo json_encode(array("success" => true, "message" => "Entrada registrada exitosamente"));
    } else {
        echo json_encode(array("success" => false, "message" => "Error al registrar entrada: " . $conn->error));
    }
} else {
